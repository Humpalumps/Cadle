...
> **SOURCE INVARIANTS (run first, takes one second): `node tools/invariants.mjs`.** It greps the source for the handful of rules that encode bugs which have shipped repeatedly — the single pointer-lock path (`Input.lock`), the absolute grass emissive ceiling, the grass roughness floor. Run it before you start and again before you report; it needs no dev server, so it works even when the harness is contended. These rules exist because prose in this file kept being followed *literally* while a new code path reintroduced the same bug — do not edit `tools/` to satisfy them, fix the code, or say in your report that the rule is wrong.
>
...
>
> **ARCHITECTURAL LAW — GROUND COVER IS NEVER EMISSIVE (this bug has shipped four times; each time a different system caused it).**
