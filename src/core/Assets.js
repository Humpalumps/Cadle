...
// RECORDED AUDIO IS OPTIONAL, AND THIS REPOSITORY DOES NOT SHIP IT (see NOTICE). The recorded shot,
// explosion and region-theme takes were generated by a third-party AI service whose licence covers
// USING them but not redistributing them, so they are not in the tree and not in its history.
// The game does not need them: every recipe in audio/sfx.js falls through to full synthesis when a
// buffer is absent (`bufShot` returns 0), and audio/music.js drops to its generative field score when
// a theme buffer is missing (`_themeKey` -> synth mode). Silence is never a failure mode here.
// A build that DOES have the takes drops the mp3s in place and adds `public/assets/audio.json`:
//   { "sfx": ["shot-handcannon-1", ...], "music": ["field-theme"], "musicDeferred": ["wood-theme"] }
// One optional fetch decides it, so neither build carries the other's dead requests.
const AUDIO = {};
const AUDIO_DEFER = {};
const audioManifest = async () => {
  try {
    const r = await fetch('/assets/audio.json');
    if (!r.ok) return;                                   // the normal case in the public build
    const m = await r.json();
    for (const n of m.sfx ?? []) AUDIO[n] = `/assets/sfx/${n}.mp3`;
    for (const n of m.music ?? []) AUDIO[n] = `/assets/music/${n}.mp3`;
    for (const n of m.musicDeferred ?? []) AUDIO_DEFER[n] = `/assets/music/${n}.mp3`;
  } catch (e) { /* no recorded audio; the synth paths cover everything */ }
};
...
export class Assets {
  ...
  async init() {
    ...
    await audioManifest();   // decides whether there IS any recorded audio, before the bar's total is fixed
