 ...
 *   makeAtlas()  -> { texture, cols:4, rows:2 }  tiles: 0 glow, 1 spark, 2 smoke, 3 ring, 4 star, 5 rune, 6 dirt, 7 flare
 *   makeDecals() -> 2x1 atlas: 0 bullet hole, 1 scorch
 *   makeSigil()  -> 512px rune circle (sigil/heal/levelup ground glyphs)
 */
export const TEX = { GLOW: 0, SPARK: 1, SMOKE: 2, RING: 3, STAR: 4, RUNE: 5, DIRT: 6, FLARE: 7 };
...
export function makeAtlas(seed = 1) {
  const c = canvas(T * 4, T * 2), ctx = c.getContext('2d');
  ...
  fillTile(ctx, 2, 0, (x, y, r) => {
    ...
    return edge * clamp01(0.25 + 0.85 * n + 0.25 * n2 - 0.25 * r);
  });
  ...
  // layout (index = col + row*4): glow(0,0) spark(1,0) smoke(2,0) ring(3,0) | star(0,1) rune(1,1) dirt(2,1) flare(3,1)
  return { texture: tex(c), cols: 4, rows: 2 };
}
