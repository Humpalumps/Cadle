export const meta = {
  name: 'aetherfall-wave5',
  description: 'Wave 5: fix-first from stored verdicts + mandatory regression gate, then fresh critics until WIN',
  phases: [
...
const COMMON = `Project: ~\\Desktop\\FPS3 (browser FPS-RPG "Aetherfall", Three.js). FIRST read CLAUDE.md and tools/BUILDER.md in full and follow them exactly. Dev server already runs at http://127.0.0.1:5173/. ALSO read ASSETS.md (AI-generated textures/SFX/music under public/assets/ — use them where the manifest says, blend with procedural detail, request missing ones with "ASSET ASK: <what>" in your report) and TECHNIQUES.md (ranked, license-verified open-source three.js techniques — follow the STEAL guidance for your piece).  MANDATORY REGRESSION GATE: run 'node tools/gate.mjs' and get exit 0 BEFORE you report done (it checks, at q=high AND q=low: no washed-white blobs in the meadow, no frozen-world rendering jitter, pointer lock engages + re-acquires). If your piece can affect any of those, run it early, not just at the end. Never weaken gate thresholds or edit tools/ - they are orchestrator-owned; if you believe the gate is wrong, say so in your report instead. Work autonomously, no questions. Use the harness to look at your actual result and iterate until it meets the bar; don't stop at "it runs".`
...
const criticPrompt = (p, n) => `Project: ~\\Desktop\\FPS3. Read tools/CRITIC.md in full and follow it exactly; also read CLAUDE.md. You are critic #${n} for PIECE "${p.key}" (files: ${p.files}). Piece brief (what the builder was asked to deliver): ${p.brief}

Before judging, run 'node tools/gate.mjs'. If it exits non-zero, the piece CANNOT be WIN - report the gate failure as the biggest_gap (verdict LOSE) unless the failure is clearly owned by another piece, in which case note it in errors[] and judge normally.
Inspect the actual running game with tools/inspect.mjs (use --name critic5-${p.key}-${n}), look at the screenshots, read report.json, then do the blind side-by-side against the real Destiny 2 / FF14 equivalent and return the JSON verdict. Be brutal and specific.`
...
"strengths": [
"Waystone landmark genuinely strong: floating faceted crystal, rotating glyph rings, orbiting shards, lantern-lined approach; the night shot with aurora is near toss-up quality for that one vista",
...
"run-2 plinth full-res trace t=13889-14414: y 7.386→7.587 (~100 ms), 7.587→7.937 (~100 ms), 7.937→8.337 (~130 ms), each eased, speed 6.5 throughout — no snap; burst-plinth-0/1.png show the waystone plinth steps being climbed",
...
"Time-of-day grading genuinely evokes FF14: warm saturated golden hour (shot-golden-lake), violet dusk, blue night with readable exposure lift (shot-night-sky with aurora + glowing waystone)",
